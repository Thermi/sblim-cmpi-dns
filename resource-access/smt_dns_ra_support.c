...
static void *parseZones(int pipeid){
	...
	/* 
	   This value is too big because of yast slowness
	   When we got free of yast we MUST reduce this value! -- Murillo
	*/
...
BINDOPTS *ReadOptions(){
	...
	char *script = my_script_path(GETOPTS);
...
ACL *ReadACL(void){
#endif
 	char *script = my_script_path("smt_dns_ra_readacl.pl");	
	char *filename = get_bindconf();
...
int WriteACL(ACL *acl){
	...
	char *uselib;
	ACL *aclptr = acl;
	asprintf(&uselib,"use lib '%s';use smt_dns_ra_acl;\n",my_script_path(""));	
	...
	SV *status = eval_pv("${smt_dns_ra_acl::WriteAcl({},$acl)};",TRUE);
